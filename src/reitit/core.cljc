...
(defn trie-router
  ...
  ([compiled-routes opts]
   (let [compiler nil ;;(::trie/trie-compiler opts (trie/compiler)) ;;
         names nil ;; (impl/find-names compiled-routes opts) ;;
         [pl nl] [nil nil]
         #_(reduce
                   (fn [[pl nl] [p {:keys [name] :as data} result]]
                     (let [{:keys [path-params] :as route} (impl/parse p opts)
                   ...
                   [nil {}]
                   compiled-routes)
         matcher (trie/compile pl compiler) ;;klm blowup
...
(defn mixed-router
  ...
  ([compiled-routes opts]
   (let [{wild true, lookup false} (group-by (impl/->wild-route? opts) compiled-routes)
         ...
         wildcard-router (trie-router wild opts) ;;3 klmlast blowup
...
(defn quarantine-router
  ...
  ([compiled-routes opts]
   (let [conflicting-paths nil ;; (impl/conflicting-paths (or (::path-conflicting opts) (impl/path-conflicting-routes compiled-routes opts)))
         conflicting? nil ;; #(contains? conflicting-paths (first %))
         {conflicting true, non-conflicting false} (group-by conflicting? compiled-routes)
         linear-router nil ;;(linear-router conflicting opts)
