...
(defn resource-response
  ...
  ([path options]
   ...
   (let [path      (-> (str "/" path)  (.replace "//" "/"))
     ...
     (println "ring.util.response resource-response path: " path)
     (println "ring.util.response resource-response root+path: " root+path)
     (println "ring.util.response resource-response fn: " io-resource-fn)
     (println "ring.util.response resource-response load+: "
              (when (not= path "/") (slurp (load root+path))))
